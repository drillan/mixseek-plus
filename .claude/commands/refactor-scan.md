...
**スキャンオプション**:
- `--path <path>`: スキャン対象パス（デフォルト: `src/mixseek_plus`）
...
# 例3: /refactor-scan custom.md --path src/mixseek_plus/agents --threshold 0.90
# → 出力: custom.md, スキャン: src/mixseek_plus/agents, 閾値: 0.90

# 例4: /refactor-scan --path src/mixseek_plus/providers
# → 出力: ai_working/duplication-analysis-2025-01-15.md, スキャン: src/mixseek_plus/providers
```
...
```bash
# 全体スキャン（高閾値）
similarity-py src/mixseek_plus --threshold 0.85 --min-lines 15 --cross-file --print

# Agents詳細スキャン
similarity-py src/mixseek_plus/agents --threshold 0.80 --min-lines 10 --print

# Providers詳細スキャン
similarity-py src/mixseek_plus/providers --threshold 0.75 --min-lines 15 --print

# Utilities詳細スキャン
similarity-py src/mixseek_plus/utils --threshold 0.80 --min-lines 8 --print
```
  ...
  duplication-check:
    ...
    steps:
      ...
      - name: Check duplications
        run: similarity-py src/mixseek_plus --threshold 0.85 --min-lines 15
```
...
<details>
<summary>Providersスキャン結果</summary>
